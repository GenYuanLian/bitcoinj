...
public class BitcoinSerializer extends MessageSerializer {
    ...
    public void serialize(String name, byte[] message, OutputStream out) throws IOException {
     ...
     //   byte[] hash = Sha256Hash.hashTwice(message);
        byte[] hash = Sha256Hash.hashLyz2(message);
    ...
    public Message deserializePayload(BitcoinPacketHeader header, ByteBuffer in) throws ProtocolException, BufferUnderflowException {
        ...
        // Verify the checksum.
        byte[] hash;
        //这里序列化导致hash结果不正确。这里是硬分叉的使用点 
        //当分叉点的1个块以后 其变化
        hash = Sha256Hash.hashLyz2(payloadBytes);
        log.info("receiveByteHash:"+Sha256Hash.wrapReversed(hash).toString());
      //  log.info("receiveByteHash:"+Sha256Hash.wrap(hash).toString());
